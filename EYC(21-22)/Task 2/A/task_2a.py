...
def get_berry(img, x, y):
    pnt = img[y][x] # BGR
    colors = ["Strawberry", "Blueberry", "Lemon"]
    values = [[0, 0, 255], [255, 0, 0], [0, 255, 255]]
    clr = colors[0]
...
def transform_vision_sensor_depth_image(vision_sensor_depth_image, image_resolution):
	...
	transformed_depth_image.resize([image_resolution[1], image_resolution[0]])
...
def detect_berries(transformed_image, transformed_depth_image):
	...
	# get centroid coordinates of each circle using transformed_image then get distance using these coordinates on transformed_depth_image
	centroids = get_berries_centroids(transformed_image)
	for centroid in centroids:
		x = centroid[0]
		y = centroid[1]
		berry = get_berry(transformed_image, x, y)
		berries_dictionary[berry].append((x, y, transformed_depth_image[y][x]))
...
def detect_berry_positions(berries_dictionary):
	...
	# since image is of size 512x512 we need to shift centroid values by [(x-256)/512, (y-256)/512)]

	for berry_name in berries_dictionary:
		berries = berries_dictionary[berry_name]
		for berry in berries:
			h = (berry[0]-256)/512
			v = (berry[1]-256)/512
			dis = (berry[2]*2) + 0.03 # since range of vision sensor in 2 meters adding 0.03 for margin
			berry_positions_dictionary[berry_name].append((h, v, dis))
			# berry_positions_dictionary[berry_name].append((round(h, 2), round(v, 2), round(dis, 2)))
