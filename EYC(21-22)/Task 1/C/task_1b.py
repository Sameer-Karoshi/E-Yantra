...
def start_simulation(client_id):
	...
	return_code = sim.simxStartSimulation(client_id, sim.simx_opmode_oneshot_wait) # Start simulation
	sim.simxGetPingTime(client_id) # wait for last command send to arrive
...
def get_vision_sensor_image(client_id):
	...
	rc, v_sensor = sim.simxGetObjectHandle(client_id, 'vision_sensor', sim.simx_opmode_oneshot_wait) # Get vision sensor handle
	if rc == sim.simx_return_ok:
		return_code, image_resolution, vision_sensor_image = sim.simxGetVisionSensorImage(client_id, v_sensor, 0, sim.simx_opmode_oneshot_wait) # Get vision sensor image array 
...
def transform_vision_sensor_image(vision_sensor_image, image_resolution):
	...
	if len(image_resolution)==2:
		transformed_image = np.array(vision_sensor_image, dtype=np.uint8) # Convert list to numpy array
		transformed_image.resize([image_resolution[1], image_resolution[0], 3]) # Convert 1D image to 3D
		transformed_image = cv2.cvtColor(transformed_image, cv2.COLOR_RGB2BGR) # Convert BGR to RGB
		transformed_image = cv2.flip(transformed_image, 0) # Flip img about X axis
...
def exit_remote_api_server(client_id):
	...
	sim.simxGetPingTime(client_id) # Wait for last command to arrive
	sim.simxFinish(client_id) # End communication thread
...
def detect_qr_codes(transformed_image):
	...
	qrs = decode(transformed_image)

	for qr in qrs:
		data = qr.data.decode("utf-8") # Get message
		c_x = qr.rect.left + (qr.rect.width/2) # X coordinate of centroid
		c_y = qr.rect.top + (qr.rect.height/2) # Y coordinate of centroid
		qr_codes.append([data, (c_x, c_y)])
